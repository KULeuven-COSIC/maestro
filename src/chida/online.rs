...
pub fn aes128_no_keyschedule(party: &mut Party, inputs: VectorAesState, round_key: Vec<AesState>) -> MpcResult<Vec<AesState>> {
...
mod test {
    use rand::thread_rng;
    use crate::chida::online::{sub_bytes, VectorAesState};
    use crate::party::Party;
    ...
    fn sub_bytes_() {
        ...
        let inputs: Vec<_> = (0..256).map(|x| secret_share(&mut rng, &GF8(x as u8)))
            .collect();
        let s1: Vec<_> = inputs.iter().map(|(s1,s2,s3)|s1.si).collect();
        let s2: Vec<_> = inputs.iter().map(|(s1,s2,s3)|s2.si).collect();
        let s3: Vec<_> = inputs.iter().map(|(s1,s2,s3)|s3.si).collect();
