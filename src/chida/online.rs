...
pub fn output_round(party: &mut Party, to_p1: &[RssShare<GF8>], to_p2: &[RssShare<GF8>], to_p3: &[RssShare<GF8>]) -> MpcResult<Vec<GF8>> {
    let (my, siii) = match party.i {
        0 => {
            // send my share to P2
            party.io().send_field::<GF8>(Direction::Next, to_p2.iter().map(|rss| &rss.si));
            // receive s3 from P3
        ...
        1 => {
            // send my share to P3
            party.io().send_field::<GF8>(Direction::Next, to_p3.iter().map(|rss| &rss.si));
            // receive s1 from P1
        ...
        2 => {
            // send my share to P1
            party.io().send_field::<GF8>(Direction::Next, to_p1.iter().map(|rss| &rss.si));
            // receive s2 from P2
...
fn mul(party: &mut Party, ci: &mut [GF8], cii: &mut [GF8], ai: &[GF8], aii: &[GF8], bi: &[GF8], bii: &[GF8]) -> MpcResult<()> {
    ...
    party.io().send_field::<GF8>(Direction::Previous, ci.iter());
