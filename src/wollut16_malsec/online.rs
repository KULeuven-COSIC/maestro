...
fn verify_dot_product<F: Field + Copy>(
    party: &mut WL16ASParty<F>,
    x_vec: &[RssShare<F>],
    y_vec: &[RssShare<F>],
    z_vec: &[RssShare<F>],
) -> MpcResult<bool>
    ...
    ChaCha20Rng: FieldRngExt<F>,
{
    ...
    let zs: Vec<F> = x
        .iter()
        .zip(y)
        .map(|(x, y)| x.si * y.si + (x.si + x.sii) * (y.si + y.sii))
        .collect();
    ...
    z_i.iter_mut()
        .enumerate()
        .for_each(|(j, z_i)| *z_i = zs[j] + alphas[j]);
    party
        .inner
        .io()
        .send_field::<F>(Direction::Previous, z_i.iter(), z_i.len());
    party
        .inner
        .io()
        .receive_field_slice(Direction::Next, &mut z_ii)
        .rcv()?;
    party.inner.io().wait_for_completion();
    let z_rss: Vec<RssShare<F>> = z_i
        .iter()
        .zip(z_ii)
