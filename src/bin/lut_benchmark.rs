...
use dist_dec::party::{ArithmeticBlackBox, error::MpcResult};
use dist_dec::{chida::ChidaParty, wollut16};
use dist_dec::share::{gf4::GF4, RssShare, bs_bool16::BsBool16, Field};
...
impl LUTBenchResult {
    ...
    fn min_avg_max(v: Vec<f64>) -> (f64, f64, f64) {
        let min = v.iter().copied().reduce(f64::min).or(Some(f64::NAN)).unwrap();
        let avg = v.iter().copied().sum::<f64>() / v.len() as f64;
        let max = v.iter().copied().reduce(f64::max).or(Some(f64::NAN)).unwrap();
        (min, avg, max)
    }

    fn print(&self, name: &str) {
        let prep_time = self.prep_time.iter().map(|d| d.as_secs_f64()).collect_vec();
        let online_time = self.online_time.iter().map(|d| d.as_secs_f64()).collect_vec();
        ...
        println!("{}\n\tPre-Processing:\t{}s (min) {}s (avg) {}s (max)", name, prep_min, prep_avg, prep_max);
        println!("\tOnline\t\t{}s (min) {}s (avg) {}s (max)", online_min, online_avg, online_max);
    }
}
...
fn lut16_ohv_as_bytes_benchmark(n_rep: usize, batch: usize) -> LUTBenchResult {
    let program = || {
        move |p: &mut ChidaParty| {
            ...
            LUTBenchResult {prep_time: prep_times, online_time: online_times}
        }
    };
    ...
    let program = || {
        move |p: &mut ChidaParty| {
            ...
            for _ in 0..n_rep {
                ...
                prep_times.push(prep_now.elapsed());
                let online_now = Instant::now();
                let res = wollut16::online::lut_with_rnd_ohv_bitsliced(&rnd_ohv, inputs1, inputs2, inputs3);
                online_times.push(online_now.elapsed());
                criterion::black_box(res);// to prevent optimization that removes the computation
            }
            LUTBenchResult {prep_time: prep_times, online_time: online_times}
...
pub fn generate_random_ohv16_bytes<P: ArithmeticBlackBox<BsBool16> + ArithmeticBlackBox<GF4>>(party: &mut P, n: usize) -> MpcResult<Vec<RndOhvOutputBytes>> {
...
fn generate_ohv16_bytes<P: ArithmeticBlackBox<BsBool16> + ArithmeticBlackBox<GF4>>(party: &mut P, n: usize, r0: Vec<RssShare<BsBool16>>, r1: Vec<RssShare<BsBool16>>, r2: Vec<RssShare<BsBool16>>, r3: Vec<RssShare<BsBool16>>) -> MpcResult<Vec<RndOhvOutputBytes>> {
    let n16 = r0.len();
    ...
    debug_assert_eq!(n16, r3.len());
     ...
     let ohv_transposed = un_bitslice_bytes([ohv_0, ohv_1, ohv_2, ohv_3, ohv_4, ohv_5, ohv_6, ohv_7, ohv_8, ohv_9, ohv_10, ohv_11, ohv_12, ohv_13, ohv_14, ohv_15]);
...
#[inline]
pub fn lut_with_rnd_ohv_bytes(rnd_ohv: &[RndOhvOutputBytes], ci: Vec<GF4>, cii: Vec<GF4>, ciii: Vec<GF4>) -> (Vec<GF4>, Vec<GF4>) {
...
mod test {
    ...
    fn ohv16_bytes() {
        ...
        let program = |b0: Vec<RssShare<BsBool16>>, b1: Vec<RssShare<BsBool16>>, b2: Vec<RssShare<BsBool16>>, b3: Vec<RssShare<BsBool16>>| {
            move |p: &mut ChidaParty| {
                generate_ohv16_bytes(p, 16, b0, b1, b2, b3).unwrap()
            }
        };
    ...
    fn random_ohv16_bytes() {
        ...
        let program = || {
            |p: &mut ChidaParty| {
                generate_random_ohv16_bytes(p, N).unwrap()
            }
        };
