...
impl_benchmark_protocol!(
    ...
    |conn: ConnectedParty, n_worker_threads: Option<usize>| GF4CircuitASParty::setup(conn, n_worker_threads, MultCheckType::Recursive { check_after_sbox: false, use_gf4p4_check: false }).unwrap(), // setup
...
impl_benchmark_protocol!(
    ...
    |conn: ConnectedParty, n_worker_threads: Option<usize>| GF4CircuitASParty::setup(conn, n_worker_threads, MultCheckType::Recursive { check_after_sbox: false, use_gf4p4_check: true }).unwrap(), // setup
    |party: &mut GF4CircuitASParty| party, // get ABB<GF8>
    None, // no preprocessing
    |party: &mut GF4CircuitASParty| party.finalize() // do finalize checks
);
